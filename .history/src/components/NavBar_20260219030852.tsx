...
const NavBar = ({logo,nav_links,bar_icon,cancel_icon}:NavProps) => {
    ...
    return (
        <div>
            <nav className="flex justify-between pl-5 p items-center fixed top-0
            w-[calc(100%-56px)] left-5 bg-background-color">
                <div className="">
            ...
            <div className={`${show ? "block" :"hidden"} absolute top-16.5 left-8`}>
                <ul className="">
                        {nav_links?.map((link,index)=>{
                            return <li key={index}>
                                <NavLink className="text-primary-color text-[18px] font-medium" 
